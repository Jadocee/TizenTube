...
function processResponse(r: any, sourceText?: unknown): any {
    ...
    try {
        ...
        if (r?.streamingData?.adaptiveFormats && configRead('videoPreferredCodec') !== 'any') {
            ...
            const hasPreferredCodec = r.streamingData.adaptiveFormats.find((format: any) =>
        ...
        if (
            r?.contents?.tvBrowseRenderer?.content?.tvSurfaceContentRenderer?.content?.gridRenderer
                ?.items
        ) {
            const grid =
                r.contents.tvBrowseRenderer.content.tvSurfaceContentRenderer.content.gridRenderer;
        ...
        if (r.messages && Array.isArray(r.messages) && !configRead('enableYouThereRenderer')) {
            r.messages = r.messages.filter((msg: any) => !msg?.youThereRenderer);
        }
        ...
        if (r?.continuationContents?.gridContinuation?.items) {
            ...
            r.continuationContents.gridContinuation.items = dropHidden(
            ...
            r.continuationContents.gridContinuation.items = hideVideo(
                r.continuationContents.gridContinuation.items,
            );
        }
    ...
    if (r?.contents?.singleColumnWatchNextResults?.results?.results?.contents && configRead('enableChapters')) {
      ...
      resolveCommand({
        ...
        if (r?.playerOverlays?.playerOverlayRenderer) {
            if (r.playerOverlays.playerOverlayRenderer.timelyActionRenderers) {
                r.playerOverlays.playerOverlayRenderer.timelyActionRenderers =
                    r.playerOverlays.playerOverlayRenderer.timelyActionRenderers.filter(
                        (a: any) =>
                ...
                if ((window?.sponsorblock as SponsorBlockSegments | null | undefined)?.segments) {
                    for (const segment of (window.sponsorblock as SponsorBlockSegments).segments!) {
                        if (manualSkippedSegments.includes(segment.category)) {
                            ...
                            r.playerOverlays.playerOverlayRenderer.timelyActionRenderers.push(
                                timelyActionData,
                            );
                        }
                    }
        ...
        if (
            ...
            if ((window?.sponsorblock as SponsorBlockSegments | null | undefined)?.segments) {
                const category = (window.sponsorblock as SponsorBlockSegments).segments!.find(
...
function processShelves(shelves: any[], shouldAddPreviews = true) {
    ...
    for (let i = shelves.length - 1; i >= 0; i--) {
        ...
        if (shelve.shelfRenderer) {
            ...
            shelve.shelfRenderer.content.horizontalListRenderer.items = dropHidden(
...
function suppressionRows(tile: any): any[] {
    ...
    if (offered.video) {
        ...
        rows.push(
            MenuServiceItemRenderer(t('longPress.hideVideo'), {
                clickTrackingParams: null,
    ...
    if (offered.channel) {
        rows.push(
            MenuServiceItemRenderer(
                t('longPress.hideChannel', {
                    channel: identity.channel?.name || identity.channel?.handle || '',
                }),
                {
                    clickTrackingParams: null,
                    playlistEditEndpoint: {
                        customAction: {
                            ...
                            parameters: {
                                entry: channelEntry(identity.channel),
                                videoId: identity.videoId,
                            },
                        },
                    },
...
function addLongPress(items: any[]) {
    ...
    for (const item of items) {
        ...
        const copiedItem = cloneJson(item);
        const subtitleNode =
            copiedItem.tileRenderer.metadata.tileMetadataRenderer.lines?.[0]?.lineRenderer
...
function hideVideo(items: any[]) {
    return items.filter((item) => {
        ...
        const progressBar = item.tileRenderer.header?.tileHeaderRenderer?.thumbnailOverlays?.find(
            (overlay: any) => overlay.thumbnailOverlayResumePlaybackRenderer,
...
try {
    if (!localStorage.getItem(HIDE_WATCHED_MIGRATION_KEY)) {
        if (
            configRead('hideWatchedVideosPages').length > 0 &&
