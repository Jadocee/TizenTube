...
Thirty-nine regression harnesses. They exist because the things that break TizenTube
...
| `release-gate/npm.test.mjs` | `.github/scripts/npm-gate.sh`, which decides whether a run publishes the npm package TizenBrew installs. An npm version cannot be un-taken and cannot meaningfully be unpublished after 72 hours, so a gate that fires when it should not is unrecoverable in a way a bad GitHub release is not. The registry is stubbed on `PATH` rather than reached, and the stub records its argv, so "which coordinate did the gate ask about" is asserted rather than assumed. Covers an unpublished version publishing even unchanged (the case the old version-changed rule could not reach, which is why nothing was ever published), a version already on the registry refusing, an unreachable registry falling back to the version diff, a reply naming some other version not counting as an answer, no `npm` on `PATH` at all, pull requests, tag pushes, a new branch, a shallow checkout — that a missing or half-built `dist/` fails the run rather than publishing a manifest naming files the tarball does not contain — and the workflow shape around it: the publish steps gated on the gate and on a configured token, nothing published before the harnesses run, and the DIAL service naming the same package `package.json` publishes under. |
