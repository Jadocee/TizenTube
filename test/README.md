...
| `release-gate/npm.test.mjs` | `.github/scripts/npm-gate.sh`, which decides whether a run publishes the npm package TizenBrew installs. An npm version cannot be un-taken and cannot meaningfully be unpublished after 72 hours, so a gate that fires when it should not is unrecoverable in a way a bad GitHub release is not. Covers a bump, an unchanged version, pull requests, tag pushes, a new branch, a shallow checkout — and that a missing or half-built `dist/` fails the run rather than publishing a manifest naming files the tarball does not contain. |
