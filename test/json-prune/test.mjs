...
check(
    '  ...and the survivors are the real ones',
...
prune(payload, AD_RULES);
check(
    'playerAds becomes false at any depth',
    `${payload.playerAds}${payload.watchNext.playerAds}`,
    'falsefalse',
);
check(
    'adSlots is emptied at any depth',
    payload.adSlots.length + payload.watchNext.adSlots.length,
    0,
);
        ...
        tvBrowseRenderer: {
            content: {
                tvSurfaceContentRenderer: {
                    content: {
                        gridRenderer: {
                            items: [{ tileRenderer: { id: 'real' } }, { adSlotRenderer: {} }],
                        },
                    },
                },
            },
...
prune(payload, AD_RULES);
const grid =
    payload.contents.tvBrowseRenderer.content.tvSurfaceContentRenderer.content.gridRenderer.items;
            ...
            shelfRenderer: {
                content: {
                    horizontalListRenderer: {
                        items: [{ tileRenderer: {} }, { adSlotRenderer: {} }],
                    },
                },
            },
        },
...
check(
    '  ...and so does the shelf inside it',
    payload.contents[0].shelfRenderer.content.horizontalListRenderer.items.length,
    1,
);
...
check(
    '  ...and the real reel survives',
    payload.entries[0].command.reelWatchEndpoint.videoId,
    'keep',
);
...
check(
    'tokens include the real keys',
    tokens.includes('adPlacements') && tokens.includes('adSlotRenderer'),
    true,
);
