...
const shapes = {
  ...
  'no whos_watching_fullscreen_zero_accounts': (() => { const o = full(); delete o.data.data.whos_watching_fullscreen_zero_accounts; return JSON.stringify(o); })(),
  'no account-selector entry': (() => { const o = full(); delete o.data.data['startup-screen-account-selector-with-guest']; return JSON.stringify(o); })(),
...
for (const [label, raw] of Object.entries(shapes)) {
  for (const enabled of [false, true]) {
    for (const perma of [false, true]) {
      ...
      if (raw !== undefined) globalThis.localStorage['yt.leanback.default::recurring_actions'] = raw;
      ...
      try {
        ...
        throws++;
        console.log(`  THROW ${label.padEnd(38)} ${mode}  -> ${e.constructor.name}: ${e.message.slice(0,60)}`);
      }
    }
  }
}
