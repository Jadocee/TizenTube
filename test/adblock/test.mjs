...
const nativeParse = JSON.parse, nativeStringify = JSON.stringify;
let fail = 0;
...
function installWrappers() {
  const ourParse = function () { return nativeParse.apply(this, arguments); };
  const ourStringify = function () { return nativeStringify.apply(this, arguments); };
  ourParse.__tt = true; ourStringify.__tt = true;
  JSON.parse = ourParse; JSON.stringify = ourStringify;
}
...
function run(scenario) {
  return new Promise((resolve) => {
    JSON.parse = nativeParse; JSON.stringify = nativeStringify;
    installWrappers();
    globalThis.window = {};
    scenario();
...
let late;
await run(() => {
  setTimeout(() => { late = yttvModule(); globalThis.window._yttv = { a: late }; }, 600);
});
check('registry appears late: module parse is patched', late.JSON.parse.__tt === true, true);
check('registry appears late: module stringify is patched', late.JSON.stringify.__tt === true, true);

// 3. A module created much later (a surface opened after boot).
let later;
await run(() => {
  globalThis.window._yttv = { a: yttvModule() };
  setTimeout(() => { later = yttvModule(); globalThis.window._yttv.b = later; }, 1200);
});
...
let after;
await run(() => {
  const frozen = Object.freeze({ JSON: Object.freeze({ parse: nativeParse, stringify: nativeStringify }) });
  after = yttvModule();
