...
// Only the rows of the HARNESSES table, which all start at one indent level.
const rows = runner.split('\n').filter((line) => line.startsWith('    { name:'));
const count = rows.length;
const types = rows.filter((r) => r.includes('types: true')).length;
const browser = rows.filter((r) => r.includes('browser: true')).length;
const bundle = rows.filter((r) => r.includes('needsBundle: true')).length;
...
const HARNESS_CLAIM = new RegExp(`\\b(\\d+|${NUMBER_WORDS})\\s+(?:regression\\s+|browser\\s+)?harnesses`, 'gi');
...
const wrongIn = (label, text) => {
    const valid = new Set([...spellings(count), ...spellings(types), ...spellings(browser), ...spellings(bundle)]);
    ...
    if (bad.length) console.log(`        ${label} claims: ${bad.join(', ')}  (real: total=${count} types=${types} browser=${browser} bundle=${bundle})`);
    return bad;
};
