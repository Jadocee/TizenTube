...
globalThis.window = {
    addEventListener: (type, cb) => {
        if (type === 'hashchange') hashListeners.push(cb);
    },
};
...
tick(300);
check(
    'video A takes its channel rule',
...
tick(300);
check(
    '  ...and takes the global default once its own channel lands',
    JSON.stringify(lastCommand()),
...
check(
    '  ...and channelOf(null) still answers for the settings row',
    context.channelOf(null)?.id,
    'UChhh',
);
