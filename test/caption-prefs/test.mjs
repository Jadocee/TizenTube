...
// --- the two commands -------------------------------------------------------
check(
    'on uses the default track',
    JSON.stringify(captionsOnCommand()),
    '{"selectSubtitlesTrackCommand":{"useDefaultTrack":true}}',
);
...
check(
    'off is an empty payload',
    JSON.stringify(captionsOffCommand()),
    '{"selectSubtitlesTrackCommand":{}}',
);
check(
    '  ...carrying none of the named arms',
    Object.keys(captionsOffCommand().selectSubtitlesTrackCommand).length,
    0,
);
check('leave issues nothing at all', commandFor('leave'), null);
check('junk issues nothing', commandFor('nonsense'), null);
check(
    'on maps to the on command',
    JSON.stringify(commandFor('on')),
    JSON.stringify(captionsOnCommand()),
);
check(
    'off maps to the off command',
    JSON.stringify(commandFor('off')),
    JSON.stringify(captionsOffCommand()),
);
...
check(
    ...
    preferenceFor({ globalDefault: 'on', channel: chan }),
    'on',
);
check('  ...including leave', preferenceFor({ globalDefault: 'leave', channel: chan }), 'leave');
check(
    'a per-channel ON beats a global off',
    preferenceFor({ globalDefault: 'off', onChannels: ['UC1 A'], channel: chan }),
    'on',
);
check(
    'a per-channel OFF beats a global on',
    preferenceFor({ globalDefault: 'on', offChannels: ['UC1 A'], channel: chan }),
    'off',
);
...
check(
    'a channel in both lists resolves to on',
    preferenceFor({
        globalDefault: 'off',
        onChannels: ['UC1 A'],
        offChannels: ['UC1 A'],
...
check(
    'an unknown channel uses the default',
    preferenceFor({ globalDefault: 'off', onChannels: ['UC9 Other'], channel: chan }),
    'off',
);
check(
    'no channel at all uses the default',
    preferenceFor({ globalDefault: 'on', onChannels: ['UC1 A'], channel: null }),
    'on',
);
check(
    'a junk default degrades to leave',
    preferenceFor({ globalDefault: 'banana', channel: chan }),
    'leave',
);
...
check(
    ...
    listHasChannel(['News Other'], { id: 'UC1', name: 'News' }),
    false,
);
