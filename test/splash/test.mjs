...
if (!body.includes('getState')) {
    console.log('FAIL: could not lift useInjectorOrProxy');
    process.exit(1);
}
...
check(
    'no daemon -> navigates to the proxy',
    typeof b.navigated === 'string' && b.navigated.includes('localhost:8099/tv'),
    true,
);

const c = await run({ canConnectToDaemon: false, isConnecting: true });
check(
    'no daemon, connecting -> still the proxy',
    typeof c.navigated === 'string' && c.navigated.includes('localhost:8099/tv'),
    true,
);
