...
export const modals = [];
export function showModal(header, content, id, update) {
    modals.push({ header, content, id, update });
}
export function overlayPanelItemListRenderer(items, selectedIndex) {
    return { overlayPanelItemListRenderer: { items, selectedIndex } };
}
export function buttonItem(title, icon, commands) {
    const b = {
        compactLinkRenderer: { serviceEndpoint: { commandExecutorCommand: { commands } } },
    };
    ...
    if (icon && icon.secondaryIcon)
        b.compactLinkRenderer.secondaryIcon = { iconType: icon.secondaryIcon };
    return b;
}
export const scrollPaneRenderer = (items) => ({
    scrollPaneRenderer: { content: { scrollPaneItemListRenderer: { items } } },
});
export const overlayMessageRenderer = (simpleText) => ({
    overlayMessageRenderer: { title: { simpleText } },
});
export const QrCodeRenderer = (url) => ({
    qrCodeRenderer: { qrCodeImage: { thumbnails: [{ url }] } },
});
export const qrcode = {
    qrcode: () => ({
        addData() {},
        make() {},
        createImgTag: () => 'src="data:image/gif;base64,AAA"',
    }),
};
