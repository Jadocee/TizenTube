...
function walk(render, label, depth, path, parentMenuId) {
    if (depth > 8) {
        problems.push(`depth>8 at ${label}`);
        return;
    }
    if (menus > 400) {
        return;
    ...
    render();
    if (stubs.modals.length !== 1) {
        problems.push(`${label}: emitted ${stubs.modals.length} modals`);
        return;
    }
    ...
    if (title === ROOT_TITLE && depth > 0)
        problems.push(`${label}: submenu falls back to the root title`);
    if (modal.id) {
        const prev = seenMenuIds.get(modal.id);
        if (prev && prev !== label)
            problems.push(`menuId "${modal.id}" used by both "${prev}" and "${label}"`);
    ...
    for (const item of items) {
        ...
        for (const c of cmds) {
            if (c === null || c === undefined)
                problems.push(`${label}/${link.title?.simpleText}: null command survived`);
        }
        ...
        if (writes) {
            ...
            if (link.secondaryIcon?.iconType?.startsWith('RADIO_BUTTON')) {
                radioRows++;
                if (back && refresh) {
                    radioWithParentRefresh++;
                    if (!refresh.customAction.parameters?.options)
                        problems.push(
                            `${label}/${link.title?.simpleText}: parent refresh has no options`,
                        );
                    if (refresh.customAction.parameters?.update !== true)
                        problems.push(
                            `${label}/${link.title?.simpleText}: parent refresh is not an update`,
                        );
                    if (refresh.customAction.parameters?.menuId === modal.id)
                        problems.push(
                            `${label}/${link.title?.simpleText}: parent refresh points at its own menu`,
                        );
                    if (refresh.customAction.parameters?.menuId !== parentMenuId)
                        problems.push(
                            `${label}/${link.title?.simpleText}: parent refresh targets "${refresh.customAction.parameters?.menuId}" but was opened from "${parentMenuId}"`,
                        );
                    const pi = refresh.customAction.parameters?.selectedIndex;
                    if (typeof pi !== 'number' || pi < 0)
                        problems.push(
                            `${label}/${link.title?.simpleText}: parent refresh has a bad selectedIndex`,
                        );
                } else if (!back && refresh) {
                    radioSelfRefresh++;
                } else
                    problems.push(
                        `${label}/${link.title?.simpleText}: radio row has neither refresh path`,
                    );
            }
            continue;
        }
...
console.log(
    `radio rows=${radioRows}  refresh-parent=${radioWithParentRefresh}  refresh-self=${radioSelfRefresh}`,
);
console.log(
