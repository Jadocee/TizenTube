...
for (const line of cfg.match(/const defaultConfig = \{([\s\S]*?)\n\};/)[1].split('\n')) {
  ...
  try { stubs.store[m[1]] = eval(`(${raw})`); } catch { stubs.store[m[1]] = raw; }
}
globalThis.window = { h5vcc: undefined };

const render = (fn) => { stubs.modals.length = 0; fn(); return stubs.modals[0]; };
...
const openParams = (r) => cmds(r).find(c => c?.customAction?.action === 'OPTIONS_SHOW').customAction.parameters;
  ...
  for (const c of list) {
    if (c?.setClientSettingEndpoint) {
      for (const d of c.setClientSettingEndpoint.settingDatas) {
        ...
        stubs.configWrite(d.clientSettingEnum.item, vk === 'intValue' ? Number(d[vk]) : d[vk]);
      ...
      events.push(`OPTIONS_SHOW(${c.customAction.parameters.menuId}, update=${c.customAction.parameters.update})`);
...
const ui = render(() => { const p = openParams(row(root, 'User Interface Settings')); optionShow(p, p.update); });
const dimRow = row(ui, 'Screen Dimming');
const dim = render(() => { const p = openParams(dimRow); optionShow(p, p.update); });

const timeoutRow = row(dim, 'Dimming Timeout');
check('Dimming Timeout row shows the stored value',
      timeoutRow.compactLinkRenderer.subtitle?.simpleText, '1 minute');

const timeoutMenuParams = openParams(timeoutRow);
check('its list opens on the stored value',
      timeoutMenuParams.selectedIndex, 3);

const timeoutMenu = render(() => optionShow(timeoutMenuParams, timeoutMenuParams.update));
check('the list header names the setting',
      timeoutMenu.header.title, 'Dimming Timeout');

const fiveMin = row(timeoutMenu, '5 minutes');
check('the stored choice is the checked one',
      row(timeoutMenu, '1 minute').compactLinkRenderer.secondaryIcon.iconType, 'RADIO_BUTTON_CHECKED');
...
check('whose row now shows the NEW value',
      row(result.modal, 'Dimming Timeout').compactLinkRenderer.subtitle?.simpleText, '5 minutes');
check('and keeps focus on that row',
...
const vp = render(() => { const p = openParams(row(root, 'Video Player Settings')); optionShow(p, p.update); });
...
const codecMenu = render(() => { const p = openParams(codecRow); optionShow(p, p.update); });
const r2 = apply(cmds(row(codecMenu, 'AV01')));
check('picking AV01 writes the key adblock.js actually reads', stubs.store.videoPreferredCodec, 'av01');
check('and the row updates', row(r2.modal, 'Preferred Video Codec').compactLinkRenderer.subtitle?.simpleText, 'AV01');

// --- Scenario: Launch To on Startup can be cleared again -------------------
const launchRow = row(ui, 'Launch To on Startup');
const launchMenu = render(() => { const p = openParams(launchRow); optionShow(p, p.update); });
const r3 = apply(cmds(row(launchMenu, 'Home')));
check('picking Home sets a launch target', JSON.parse(stubs.store.launchToOnStartup).browseEndpoint.browseId, 'FEtopics');
const launchMenu2 = render(() => { const p = openParams(row(r3.modal, 'Launch To on Startup')); optionShow(p, p.update); });
check('the list now opens on Home',
      items(launchMenu2).findIndex(i => i.compactLinkRenderer.secondaryIcon.iconType === 'RADIO_BUTTON_CHECKED'),
