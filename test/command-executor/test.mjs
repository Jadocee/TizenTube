...
function routerModule(tag) {
  class Router {
    ...
    route() { return 'ytlrActionRouter:' + this.tag; }
    unrelated() { return 1; }
  }
...
const commandFunction = function () { this.actionName = 'x'; };
...
console.log('The normal case:');
{
  ...
  check('executeFunction is bound to its own instance', r.executeFunction(), 'ytlrActionRouter:a');
  ...
  check('executeFunction is a bound method, not the raw one', r.executeFunction === singleton.route, false);
}
...
console.log('\nA module that misbehaves must not abort the search:');
{
  ...
  const thrower = () => {};
  thrower.getInstance = () => { throw new Error('not ready'); };
  const empty = () => {};
  empty.getInstance = () => undefined;
  const r = withRegistry({ bad: thrower, none: empty, good: mod, cmd: commandFunction });
  check('still resolves past a throwing getInstance', r && r.executeFunction(), 'ytlrActionRouter:b');
}
...
console.log('\nThe instance and the method must come from the SAME module:');
{
  ...
  check('method still belongs to the instance it was found on', r && r.executeFunction(), 'ytlrActionRouter:real');
}
...
console.log('\nThe prototype scan must not pick constructor, or invoke accessors:');
{
  ...
  class Sneaky {
    ...
    get trap() { getterCalls++; return () => 'invoked'; }
    plain() { return 'nope'; }
  }
  ...
  check('did not bind constructor as the router', r && r.executeFunction(), 'ytlrActionRouter:good');
