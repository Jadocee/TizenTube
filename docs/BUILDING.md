...
| | Version | Why |
| --- | --- | --- |
| Node | **22.6 or newer** | thirty-three harnesses run under `--experimental-strip-types`, which lands in 22.6. On Node 20 they fail before a single assertion. CI pins 22. |
| pnpm | 10.33.0 | pinned by `packageManager` in the root `package.json`; `corepack enable` picks it up |
| Chromium | any recent | only for the eight browser harnesses. Without it they *skip*, which is fine locally and a failure in CI |
...
Eight harnesses drive real Chromium through Playwright and two of those also need
...
**Thirty-three harnesses fail before asserting anything** — Node is older than 22.6.
`node -v`.
