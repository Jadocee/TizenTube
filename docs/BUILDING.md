...
| | Version | Why |
| --- | --- | --- |
| Node | **22.6 or newer** | twenty-seven harnesses run under `--experimental-strip-types`, which lands in 22.6. On Node 20 they fail before a single assertion. CI pins 22. |
...
**Twenty-seven harnesses fail before asserting anything** — Node is older than 22.6.
`node -v`.
